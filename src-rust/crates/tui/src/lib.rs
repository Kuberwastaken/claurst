...
mod tests {
    ...
    #[test]
    fn test_turn_complete_tps_uses_flushed_len_for_providers_without_usage() {
        // Simulate: MessageStop flushes streaming_text (via mem::take), then
        // TurnComplete runs with usage.output_tokens == 0 (provider does not
        // report usage in stream). The TPS fallback must use
        // `last_flushed_stream_len` captured at flush time, not the now-empty
        // `streaming_text`.
        let mut app = make_app();
        app.streaming_text = "hello world this is a test".to_string();
        // Simulate flush (as done by MessageStop handler)
        let text = std::mem::take(&mut app.streaming_text);
        app.last_flushed_stream_len = text.len();
        // Now streaming_text is empty, but last_flushed_stream_len has the value
        assert_eq!(app.streaming_text.len(), 0);
        assert_eq!(app.last_flushed_stream_len, 26);
        // The fallback would compute 27/4 = 6 tokens (not 0)
        let fallback = (app.last_flushed_stream_len / 4).max(if app.last_flushed_stream_len == 0 {
            0
        } else {
            1
        });
