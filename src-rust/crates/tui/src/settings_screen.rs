...
impl SettingsScreen {
    ...
    fn apply_settings_from_snapshot(&mut self) {
        ...
        self.file_injection_max_size = self
            ...
            .to_string();
        self.yolo_mode = self.settings_snapshot.yolo_mode;
    }
...
fn all_entries(screen: &SettingsScreen) -> Vec<SettingsEntry> {
    let mut entries = vec![
        ...
        SettingsEntry {
            key: "yoloMode",
            label: "YOLO mode",
            description: "Skip all permission prompts and auto-approve every tool call. Sub-agents inherit this.",
            kind: SettingKind::Bool,
            value: if screen.yolo_mode { "true" } else { "false" }.to_string(),
        },
    ];
        ...
        match entry.kind {
            SettingKind::Bool => {
                ...
                match entry.key {
                    ...
                    "yoloMode" => {
                        screen.yolo_mode = new_value;
                        screen.settings_snapshot.yolo_mode = new_value;
                        let _ = screen.settings_snapshot.save_sync();
                    }
