...
const PROMPT_SLASH_COMMANDS: &[(&str, &str)] = &[
    ...
    ("help", "Show help"),
    ("hidecontent", "Toggle hiding tool content in log"),
...
pub struct App {
    ...
    /// True when the current goal has been marked complete. Used by the
    /// transcript renderer to switch the "GOAL ACTIVE" badge to
    /// "GOAL COMPLETED".
...
impl App {
    ...
    pub fn intercept_slash_command(&mut self, cmd: &str) -> bool {
        ...
        match cmd {
            ...
            "hidecontent" => {
                self.config.hide_tool_content = !self.config.hide_tool_content;
                let mut settings = Settings::load_sync().unwrap_or_default();
                settings.config.hide_tool_content = self.config.hide_tool_content;
                let _ = settings.save_sync();
                self.status_message = Some(if self.config.hide_tool_content {
                    "Tool content hidden (type only).".to_string()
                } else {
                    "Tool content visible.".to_string()
                });
                true
            }
            "effort" => {
                        ...
                        if should_submit {
                            ...
                            if crate::input::is_slash_command(&self.prompt_input.text) {
                                ...
                                if self.intercept_slash_command_with_args(cmd, args) {
                                    ...
                                    // Command needs the model but we're streaming.
                                    self.status_message = Some(
                                        "Command queued -- wait for current turn to finish."
                                            .to_string(),
                                    );
                                    self.clear_prompt();
                                    continue;
