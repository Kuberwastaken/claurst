    ...
    pub fn handle_key_event(&mut self, key: KeyEvent) -> bool {
        ...
        if self.model_picker.visible {
            match key.code {
                ...
                KeyCode::Enter => {
                    // Capture all-providers mode BEFORE confirm() — confirm()
                    // calls close() which resets all_providers_mode to false.
                    ...
                    if let Some((model_id, effort)) = self.model_picker.confirm() {
                        ...
                        let full_model = if was_all_providers {
                            let filtered = self.model_picker.filtered_models_grouped();
                            if let Some((entry, _is_fav)) = filtered.get(self.model_picker.selected_idx) {
