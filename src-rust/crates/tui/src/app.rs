...
pub struct App {
    ...
    pub plan_mode: bool,
    /// YOLO mode flag mirrored from settings for runtime toggling via `/yolo`.
    pub yolo_mode: bool,
    ...
    pub fn intercept_slash_command(&mut self, cmd: &str) -> bool {
        ...
        match cmd {
            ...
            "yolo" => {
                ...
                self.config.permission_mode = if self.yolo_mode {
                    PermissionMode::BypassPermissions
                } else {
                    PermissionMode::Default
                };
                // Persist to settings.json so it survives restarts and propagates
                // to sub-agents (they inherit permission_mode via ToolContext).
                if let Ok(mut settings) = claurst_core::Settings::load_sync() {
                    settings.yolo_mode = self.yolo_mode;
                    let _ = settings.save_sync();
                }
                self.status_message = Some(if self.yolo_mode {
                    "YOLO mode ON — all permission prompts skipped.".to_string()
                } else {
                    "YOLO mode OFF.".to_string()
                });
                true
            }
            "compact" => {
...
mod tests {
    ...
    #[test]
    fn test_yolo_slash_command_toggles_bypass() {
        use claurst_core::config::PermissionMode;
        let mut app = make_app();
        assert!(!app.yolo_mode);
        assert_eq!(app.config.permission_mode, PermissionMode::Default);
        assert!(app.intercept_slash_command("yolo"));
        assert!(app.yolo_mode);
        assert_eq!(
