...
impl App {
    ...
    fn handle_keybinding_action(&mut self, action: &str) -> bool {
        match action {
            ...
            "stashInput" => {
                // Alt+S: stash the current prompt text, or restore the
                // stashed text if already held. Disabled while streaming so
                // the stashed buffer is not clobbered.
                if self.is_streaming {
                    return false;
                }
                let text = self.prompt_input.text.clone();
                if let Some(stashed) = self.prompt_input.stashed_text.take() {
                    self.prompt_input.text = stashed;
                } else if !text.is_empty() {
                    self.prompt_input.stashed_text = Some(text);
