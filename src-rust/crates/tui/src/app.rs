...
pub struct App {
    ...
    /// Discovered skills cached for autocomplete and command palette.
...
impl App {
    ...
    /// Refresh the list of discovered skills from disk and settings.
    ...
    pub fn all_slash_commands(&self) -> Vec<(String, String)> {
        ...
        // Add custom command templates from settings
        for (name, template) in &self.config.commands {
            if !cmds.iter().any(|(n, _)| n == name) {
                let desc = template.description.as_deref().unwrap_or("Custom command");
                cmds.push((name.clone(), desc.to_string()));
            }
        }
    ...
    /// Rebuild command palette items to include built-in commands, templates, and discovered skills.
    ...
    /// Rebuild full-screen help overlay entries.
    ...
    pub fn refresh_prompt_input(&mut self) {
        ...
        let all_cmds = self.all_slash_commands();
        let cmd_refs: Vec<(&str, &str)> = all_cmds
            .iter()
