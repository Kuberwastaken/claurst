...
fn render_input(frame: &mut Frame, app: &App, area: Rect, focused: bool) {
    ...
    if let Some(status_area) = status_area {
        ...
        let right_hint = if !app.has_credentials {
            ...
            // Empty input — show ? shortcuts alongside stash hint.
            if app.prompt_input.stashed_text.is_some() {
                Line::from(vec![Span::styled(
                    "alt+s restore stashed input",
                    Style::default().fg(Color::Magenta),
                )])
            } else {
                Line::from(vec![Span::styled(
                    "? shortcuts · alt+s stash",
                    Style::default().fg(dim),
                )])
            }
        } else if app.prompt_input.stashed_text.is_some() {
            // Input present and text is stashed — prompt restore.
            Line::from(vec![Span::styled(
                "alt+s restore stashed input",
                Style::default().fg(Color::Magenta),
            )])
        } else {
            // Input present, nothing stashed — offer stash.
            Line::from(vec![Span::styled("alt+s stash", Style::default().fg(dim))])
        };
...
fn render_footer(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let left_spans: Vec<Span> = if app.voice_recording {
        ...
        // Stash indicator — shown when input text has been stashed via Alt+S.
        if app.prompt_input.stashed_text.is_some() {
            if !spans.is_empty() {
                spans.push(Span::raw("  "));
            }
            spans.push(Span::styled(
                " stash (alt+s to restore) ",
                Style::default().fg(Color::Magenta),
            ));
        }
