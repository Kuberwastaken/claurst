...
pub struct PromptInputState {
    ...
    pub text: String,
    /// Stashed text (saved via Alt+S toggle, restored on second press).
    pub stashed_text: Option<String>,
...
mod tests {
    ...
    #[test]
    fn stashed_text_starts_none() {
        let state = PromptInputState::new();
        assert!(state.stashed_text.is_none());
    }
}
