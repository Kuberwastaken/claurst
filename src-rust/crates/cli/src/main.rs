...
struct Cli {
    ...
    /// Permission mode. If omitted, the value from settings.json (e.g.
    /// `yoloMode: true`) is used. Pass explicitly to override the persisted
...
async fn main() -> anyhow::Result<()> {
    ...
    } else if let Some(pm) = cli.permission_mode {
        // Explicit --permission-mode overrides settings.json.
        config.permission_mode = pm.into();
    }
    // Otherwise: keep the permission_mode inherited from settings.json
    // (effective_config may have already set BypassPermissions via yoloMode).
