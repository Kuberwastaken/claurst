    ...
    'main: loop {
        ...
        if task_finished {
            if let Some((handle, msgs_arc)) = current_query.take() {
                ...
                if claurst_core::goals_enabled() {
                    let store = claurst_core::GoalStore::open_default();
                    app.active_goal_badge = store
                        .as_ref()
                        .and_then(|s| s.get_active_goal(&session.id))
                        .map(|_goal| "set".to_string());
                    // Check whether the goal was completed (by model or user)
                    // so the transcript badge can switch to "GOAL COMPLETED".
                    app.goal_completed = store
                        .as_ref()
                        .and_then(|s| s.get_goal(&session.id))
                        .is_some_and(|g| g.status == claurst_core::GoalStatus::Complete);
                }
            }
        }
