...
impl SlashCommand for TemplateCommand {
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        ...
        let prompt = if !args.trim().is_empty()
            && !self.template.template.contains("$ARGUMENTS")
            && !self.template.template.contains("$1")
        {
            format!("{}\n\n{}", self.template.template, args.trim())
        } else {
            self.template
                .template
...
impl SlashCommand for SkillCommand {
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        ...
        let arg2 = words.next().unwrap_or("");
        let prompt = if !args.trim().is_empty()
            && !self.template.contains("$ARGUMENTS")
            && !self.template.contains("$1")
        {
            format!("{}\n\n{}", self.template, args.trim())
        } else {
            self.template
                .replace("$ARGUMENTS", args)
                .replace("$1", arg1)
